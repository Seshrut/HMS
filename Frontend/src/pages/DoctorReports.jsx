      ...
      <main className="dashboard-main">
        ...
        {selected === null ? (
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            {reports.map(r => (
              <div key={r.id} className="dashboard-section-card" style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }} onClick={() => setSelected(r)}>
  ...
  return (
    <div style={{ marginBottom: "16px" }}>
      ...
      <ul style={{ margin: 0, paddingLeft: "20px" }}>
        {items.map((item, i) => <li key={i} style={{ fontSize: "14px", color: "#374151", marginBottom: "3px" }}>{item}</li>)}
      </ul>
    </div>
  )
}
